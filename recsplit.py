...
while True:
    ...
    if like == "y":
        ...
        count += 1
        # Adding "Yes" and "No" buttons with lambda functions
        yes_button = ttk.Button(root, text="Yes", command=lambda idv_song=idv_song: like_song(idv_song))
        yes_button.pack(pady=10)

        no_button = ttk.Button(root, text="No", command=lambda idv_song=idv_song: dislike_song(idv_song))
        no_button.pack(pady=10)
        
        
        # button_clicked.set(False)  # Reset the variable to False
        # root.wait_variable(button_clicked)  # Wait for the variable to become True
        root.wait_variable(idv_song)  # Wait for a button click before continuing
...
playlist_name_entry = ttk.Entry(root, width=30)
playlist_name_entry.pack(pady=10)

start_button = ttk.Button(root, text="Start Recommendation", command=create_playlist_and_add_tracks)
